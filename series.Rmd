...
A modelagem de Box-Jenkins, também conhecidos como modelos ARIMA, faz parte do grupo de metodologias que consistem na construção de **modelos probabilísticos**, e se baseiam fundamentalmente na análise das **autocorrelações** da série em estudo. Como já mostrado anteriormente, cada variável composta pelas observações $Z_1, ...,Z_N$, sendo $N$ o tamanho da série, será a realização de um processo estocástico.

Sendo assim, nossa principal tarefa ao aprender esses modelos será **identificar e caracterizar** o(s) processo(s) estocástico(s) que gera(m) a série em estudo.

![Figura 3: Filtro linear.](filtro.png)

A Figura 3 ilustra o procedimento geral para a ``geração'' da nossa série $Z_t$ em estudo. O filtro linear geral $\psi(B)$ é aplicado ao ruído aleatório, produzindo a série em estudo. Deste modo, cabe ao pesquisador identificar esse filtro linear. Sobre esse processo, vale ressaltar que:

* pode existir **mais de um processo** $\psi(B)$, da classe ARIMA, capaz de descrever a série em estudo $Z_t$;
* em geral, os modelos ARIMA são parcimoniosos;
* o conhecimento desses processos exigem do pesquisador um conhecimento acima da média, além do mero conhecimento sobre pacotes computacionais.

Traduzindo a Figura 3, vamos considerar que os modelos vistos nessa seção são casos particulares de um **modelo de filtro linear**. Isso significa que a série $Z_t$ será gerada por um sistema linear em que a entrada é um ruído branco $a_t$, já descrito anteriormente.

Assim, temos que:

$$Z_t = \mu + a_t + \psi_1 a_{t-1} + \psi_2 a_{t-2} + \psi_3 a_{t-3} + ...$$

Considerando o operador de defasagem $a_{t-k} = B^k a_t$, podemos escrever o modelo como segue:

$$Z_t = \mu + a_t + \psi_1 Ba_t + \psi_2 B^2a_t + \psi_3 B^3a_t + ...$$
Sendo assim:

$$Z_t = \mu + (1 + \psi_1B + \psi_2B^2 + \psi_3B^3 + ...)a_t$$
Chamamos a expressão $(1 + \psi_1B + \psi_2B^2 + \psi_3B^3 + ...)$ de $\psi(B)$, que por sua vez é conhecida como **polinômio característico** de $Z_t$. Logo, o modelo pode ser escrito como segue:

$$Z_t = \mu + \psi(B)a_t$$
A esperança e variância generalizadas de $Z_t$ podem ser demonstradas matematicamente.

Dentro do modelo de filtro linear representado anteriormente, existem diversos processos estocásticos que podem ser utilizados para descrever a série $Z_t$ em estudo, e o trabalho do estatístico/pesquisador consiste basicamente em: i. identificar o processo; ii. após a identificação, estimar os parâmetros que compõe o modelo; e iii. diagnosticar o(s) modelo (s) através da análise de resíduos. Aqui, vale que é possível (e provável) que exista mais de um processo capaz de descrever a série $Z_t$. Sendo assim, o primeiro passo consiste em conhecer os principais modelos que fazem parte da metodologia de Box-Jenkins. Vamos dividir nosso estudo da seguinte maneira:

* **Modelos para séries localmente constantes:**
  * Auto-regressivos - $AR(p)$
  * Médias móveis - $MA(q)$
  * Auto-regressivos e de médias móveis - $ARMA(p,q)$
* **Modelos para séries com tendência:**
  * Auto-regressivos Integrados de Médias Móveis - $ARIMA(p,d,q)$
* **Modelos para séries com sazonalidade:**
  * Auto-regressivos Sazonais - $SAR(P)$
  * Médias Móveis de Sazonais - $SMA(Q)$
  * Auto-regressivos e de Médias Móveis Sazonais- $SARMA(P,Q)$
* **Modelos para séries com tendência e sazonalidade:**
  * Auto-regressivos Integrados de Médias Móveis Sazonais - $ARIMA(p,d,q)(P,D,Q)_s$

Esses modelos serão explorados em termos de conceitos, esperança, funções de autocovariância, autocorrelação e autocorrelação parcial. Mas, inicialmente, é interessante apresentarmos o processo estocástico que representa o ruído de cada modelo, chamado de **ruído branco**.
